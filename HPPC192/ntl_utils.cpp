...
void InitNTL(int n, fq_ctx_t ctx){
...
bool FindRootTimeout(NTL::GF2E& root, const NTL::GF2EX& ff){
    ...
    // As FindRoot method from NTL never returns when no root is found for P(X) - Y = 0
    // Then define this new one with a brief timeout (on milli) in order to cancel the call
...
bool RootPrivatePoly(mzd_t *_private_polynomial, mzd_t *root, mzd_t *_const_term, int n, int degree, fq_ctx_t ctx){
    ...
    NTL::GF2EX inverse_F = private_polynomial - a;
    

    NTL::GF2E _root;

    while (!FindRootTimeout(_root, inverse_F))
        return false;
    
    NTL::vec_GF2 root_vec =  NTL::to_vec_GF2(NTL::conv<NTL::GF2X>(_root));
    NTL::vec_GF2 vec(NTL::INIT_SIZE, n);
